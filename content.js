...
(function () {
    ...
    function createOverlay() {
        ...
        if (document.getElementById("seia-side-element"))
            return null;
    ...
    function handleDbLevel(db) {
        if (typeof db === "number" && db >= CONFIG.thresholdDb)
            state.lastLoudTs = Date.now();
    }
    // Loudness updates relayed from the offscreen analyser (Chrome only)
    function handleMessage(msg) {
        if (msg.type === "db-level")
            handleDbLevel(msg.db);
    }
    // --- Firefox fallback ---
    // 
    ...
    function scanForMediaElements() {
        document.querySelectorAll("audio, video").forEach((el) => hookElement(el));
    }
