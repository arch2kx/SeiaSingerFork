...
const input = document.getElementById("thresholdInput") as HTMLInputElement | null;
...
const seiaSizeFill = document.getElementById("seiaSizeFill") as HTMLDivElement | null;

const SLIDER_THUMB_PX = 20;
const SLIDER_FILL_GAP_PX = 3;

function updateSliderFill(slider: HTMLInputElement, fill: HTMLDivElement) {
...
chrome.storage.local.get({ seiaSize: DEFAULT_SIZE }, ({ seiaSize }: { seiaSize: number }) => {
  if (!seiaSizeSlider || !seiaSizeFill) return;
